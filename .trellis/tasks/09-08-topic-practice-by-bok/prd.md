...
| D5 | 层级 | 数据模型存两层（5 域 + 16 能力项）；AI 打标打在能力项层；**界面只暴露 5 个域** |
...
- **R5 专项练习入口**：题库卡片提供「专项练习」按钮，弹窗列出 5 个域及「未分类」，每项显示考试出题配额与本库题数，题数为 0 的项不可选。
- **R6 专项练习会话**：选定后开始一次 `mode="topic"` 的会话，题目为该域下所有能力项关联题目的去重并集，随机序，不限数量。
...
- [ ] AC6：`GET /api/banks/{bank_id}/topics` 返回 5 个域，每项含 `blueprint_min` / `blueprint_max` / `question_count`，另含 `unclassified_count`；「至少有一个考点的题数 + unclassified_count」等于题库总题数。
- [ ] AC7：选定某域开始练习后，会话内每道题都至少关联该域下的一个能力项，且题目不重复。
...
- 跨题库、跨考试项目的整体考点练习（D9）
- 界面暴露 16 个能力项这一层（D5，数据已就位，属纯前端后续改动）
- 按考点展示做对率 / 掌握度（D15）
- 导入链路结束后自动触发打标（D16）
